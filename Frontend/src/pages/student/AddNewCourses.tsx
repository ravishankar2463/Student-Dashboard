...
function AddNewCourses({}: Props) {
  ...
  useEffect(() => {
    if (!submitting && modalState.isSubmitted) {
      if (modalState.selectedCourse.name !== "") {
        toast(`${modalState.selectedCourse.name} added to your Courses.`);
      }
      closeModal();
      navigate(
  ...
  const addCourse = (courseId: number) => {
    ...
    submit(
      ...
      {
        method: "post",
        action: `/home/${authCtx.loggedInStudent?.id}/registerNewCourse/${authCtx.loggedInStudent?.id}`,
      }
    );
  };
  ...
  return (
    <>
      <div className={`min-h-[90vh] ${backgroundColor} text-white`}>
        <h1 className="text-3xl text-center pt-2">Enlist In A New Course</h1>
        <ul
          className={`px-3 py-3 ${
          ...
          {loaderData.length == 0 ? (
            <div className="flex flex-col items-center">
              <h1 className="text-5xl pt-10">
                No Courses Remaining To Be Enlisted In
              </h1>
              <a
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-2 justify-end">
              <Button
                ...
                bgColor="black"
                type="button"
                onClick={() => addCourse(modalState.selectedCourse.id)}
              >
...
export const loader =
  ...
  async ({ params }: LoaderFunctionArgs) => {
    const allCourses = await queryClient.fetchQuery("allCourses", () =>
      getCourses()
    );

    const studentEnlistedCourses = await queryClient
      .fetchQuery(["studentRegisteredCourses", params.studentId], () =>
        getRegisteredCourses(Number(params.studentId))
      )
      .then((courseList) => courseList.map((course) => course.id));

    const filteredCourses = allCourses.filter(
      (course) => !studentEnlistedCourses.find((id) => id == course.id)
    );
  ...
  async ({ params, request }: ActionFunctionArgs) => {
    ...
    try {
      data.response = await queryClient.fetchQuery("loggedinUser", () =>
        addCourse(formData.studentId, formData.courseId)
      );
    } catch (err) {
